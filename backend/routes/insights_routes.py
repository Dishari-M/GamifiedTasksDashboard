...
def get_today_insight(date: str | None = None, user_id: str = Depends(current_local_user_id)):
    return today_insight_response(date, user_id)


@router.post("/today/generate", status_code=status.HTTP_200_OK)
def generate_today_insight(payload: TodayInsightGenerateRequest, user_id: str = Depends(current_local_user_id)):
