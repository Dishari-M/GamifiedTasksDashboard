...
def list_oracle_tasks(filters=None, user_id=DEFAULT_USER_ID):
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        return task_repository.list_tasks(cur, _user_id(user_id), filters, work_date)
...
def create_oracle_task(payload, user_id=DEFAULT_USER_ID):
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        _validate_unique_external_identity(cur, _user_id(user_id), task)
        ai = enrich_task_with_ai(task) if task.get("run_ai_enrichment") else fallback_task_enrichment(task)
        task_id = task_repository.insert_task(cur, _user_id(user_id), task, ai)
        worked_dates = _worked_dates_for_task(task)
        for work_date in worked_dates:
            task_repository.insert_work_date(cur, _user_id(user_id), task_id, work_date, task.get("estimated_minutes"))
        task_repository.insert_task_event(cur, _user_id(user_id), task_id, "TASK_CREATED", None, _event_payload(task, ai))
        conn.commit()
        return task_repository.fetch_task(cur, _user_id(user_id), task_id, _today_utc())
...
def update_oracle_task(task_id, payload, user_id=DEFAULT_USER_ID):
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        existing = task_repository.fetch_task_for_update(cur, _user_id(user_id), task_id)
        if not existing:
            _not_found()
        ...
        if fields:
            ok = task_repository.update_task_fields(cur, _user_id(user_id), task_id, fields, row_version)
        else:
            ok = task_repository.touch_task(cur, _user_id(user_id), task_id, row_version) if work_date_change_requested else True
        if not ok:
            _conflict(existing["row_version"])

        if "worked_dates" in data:
            task_repository.sync_task_work_dates(cur, _user_id(user_id), task_id, data["worked_dates"])
        if "working_today" in data:
            _set_work_date(cur, _user_id(user_id), task_id, _today_utc(), bool(data["working_today"]))
...
def update_oracle_task_today(task_id, payload, user_id=DEFAULT_USER_ID):
    ...
    try:
        ...
        if requested is None:
            worked_dates = task_repository.list_worked_dates(cur, _user_id(user_id), task_id)
        ...
        if requested and existing["status"] not in {"Blocked", "Done"}:
            ...
            fields.append(("COMPLETED_AT", "completed_at", None))
        changed = _set_work_date(cur, _user_id(user_id), task_id, work_date, requested)
        if fields or changed:
            ok = (
                task_repository.update_task_fields(cur, _user_id(user_id), task_id, fields, row_version)
                if fields
                else task_repository.touch_task(cur, _user_id(user_id), task_id, row_version)
            )
            if not ok:
                _conflict(existing["row_version"])
            task_repository.insert_task_event(
        ...
        conn.commit()
        return task_repository.fetch_task(cur, _user_id(user_id), task_id, work_date)
    except HTTPException:
        if conn:
            conn.rollback()
        raise
...
def _clear_today_after_completion(task_id, task, user_id, event_type):
    ...
    try:
        conn = get_connection()
        cur = conn.cursor()
        task_repository.delete_work_date(cur, _user_id(user_id), _task_id(task_id), _today_utc())
        task_repository.insert_task_event(
            cur,
            _user_id(user_id),
            _task_id(task_id),
        ...
        conn.commit()
        return task_repository.fetch_task(cur, _user_id(user_id), _task_id(task_id), _today_utc())
