...
def _task_row(row):
    worked_dates = _json_dates(row[29])
    task = {
        "task_id": row[0],
        "id": str(row[0]),
        "user_id": row[1],
        ...
        "due_at": row[10],
        "start_at": row[11],
        "estimated_minutes": row[12] or row[20] or 0,
        "actual_minutes": row[13] or 0,
        "xp_value": row[14] or 0,
        "notes": _text(row[15]),
        "labels": _json_array(row[16]),
        "ai": {
            "difficulty": row[17] or "Medium",
            "impact_score": row[18] or 0,
            "priority_score": row[19] or 0,
            "effort_minutes": row[20] or row[12] or 0,
            "category": row[21],
            "insight": _text(row[22]),
            "model_id": row[23],
            "enriched_at": row[24],
        },
        "completed_at": row[25],
        "created_at": row[26],
        "updated_at": row[27],
        "row_version": row[28] or 1,
        "worked_dates": worked_dates,
        "working_today": bool(row[30]),
    }
    task.update(
        {
