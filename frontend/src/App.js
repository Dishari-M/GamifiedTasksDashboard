...
const buildAiFields = (task) => {
  const priorityWeight =
    { Critical: 10, High: 8, Medium: 5, Low: 3 }[task.priority] || 5;
  const effort = Math.max(
    15,
    parseNumber(task.time ?? task.estimatedMinutes, 60),
  );
  ...
  const xp = Math.max(
    ...
    parseNumber(
      task.xp,
      Math.round((effort * 0.75 + impact * 9 + priorityWeight * 5) / 10) * 10,
    ),
  );
  const difficulty =
    effort >= 105 || priorityWeight >= 9
      ? "Hard"
      : effort <= 35 && priorityWeight <= 5
        ? "Easy"
        : "Medium";
  return {
    difficulty,
    ...
    aiInsight:
      task.aiInsight ||
      `${task.priority} priority ${task.type || "Task"} with ${formatMinutes(effort)} expected effort. ${task.notes ? "Use the notes as context for standup and overview summaries." : "Add notes as you learn more to improve the summary."}`,
  };
};
...
const initialTasks = [
  normalizeTask({
    ...
    description:
      "Users face timeout while making payments on the checkout page.",
    source: "Jira",
    type: "Bug",
    priority: "High",
    status: "In Progress",
    ...
    dueDate: todayKey(),
    notes:
      "Retry policy may be too generous for the gateway sandbox. Validate timeout cap and add regression coverage.",
  ...
  normalizeTask({
    ...
    notes:
      "Contract is clear; biggest risk is mapping courier status states cleanly.",
    labels: ["api"],
  }),
  normalizeTask({
    id: "DOC-047",
  ...
  normalizeTask({
    ...
    completedAt: nowIso(),
    notes:
      "Cache invalidation looked good. Suggested a smaller TTL for the checkout path.",
    labels: ["review"],
  }),
  normalizeTask({
    id: "OUT-902",
  ...
  >
    ...
    <div className="sidebar-card streak-card" data-testid="sidebar-streak-card">
      <div className="mini-title">
        <Fire size={20} weight="fill" aria-hidden="true" /> Streak
      </div>
    ...
    <header className="topbar" data-testid="topbar">
      ...
      <div className="topbar-title">
        ...
        <p data-testid="page-subtitle">
          Plan the work, capture the learning, and keep momentum visible.
        </p>
      </div>
...
const StatCard = ({
  ...
  <section className="stat-card surface" data-testid={testId}>
    <div className="stat-head">
      <span>{label}</span>
      <IconBadge icon={icon} tone={tone} testId={`${testId}-icon`} />
    </div>
    <div className="stat-value" data-testid={`${testId}-value`}>
      {value}
    </div>
    {typeof progress === "number" && (
      <div
        className="progress-track"
        data-testid={`${testId}-progress-track`}
    ...
    <div
      className={`stat-detail ${down ? "negative" : "positive"}`}
      data-testid={`${testId}-detail`}
    >
      {trend &&
        (down ? (
          <TrendDown size={17} weight="bold" aria-hidden="true" />
        ) : (
          <TrendUp size={17} weight="bold" aria-hidden="true" />
        ))}
      {detail}
    </div>
  </section>
...
const MissionCard = ({ task, index }) => {
  ...
  return (
    <article
      ...
      data-testid={`mission-card-${slug(task.id)}`}
    >
      <IconBadge
        icon={Icon}
        tone={task.accent}
        testId={`mission-icon-${slug(task.id)}`}
      />
      <div className="mission-copy">
        <div className="mission-title-row">
          <h3 data-testid={`mission-title-${slug(task.id)}`}>{task.title}</h3>
          <Pill
            tone={task.type.toLowerCase()}
            testId={`mission-type-${slug(task.id)}`}
          >
            {task.type}
          </Pill>
        </div>
        <p
          ...
          data-testid={`mission-meta-${slug(task.id)}`}
        >
          {task.source} - {task.id}
        </p>
        <p data-testid={`mission-description-${slug(task.id)}`}>
          {task.aiInsight || task.description}
        </p>
      </div>
      <div className="mission-score">
        <Pill
          tone={task.priority.toLowerCase()}
          testId={`mission-priority-${slug(task.id)}`}
        >
          {task.priority}
        </Pill>
        <span data-testid={`mission-time-${slug(task.id)}`}>
          <Clock size={16} weight="duotone" aria-hidden="true" /> {task.time}{" "}
          mins
        </span>
        <strong data-testid={`mission-xp-${slug(task.id)}`}>
          {task.xp} XP
        </strong>
        <span
          className="mission-rank"
          data-testid={`mission-rank-${slug(task.id)}`}
        >
          #{index + 1}
        </span>
      </div>
...
const SchedulePanel = () => (
  <section className="surface schedule-panel" data-testid="schedule-panel">
    <div className="section-heading">
      ...
      <NavLink to="/calendar" data-testid="view-calendar-link">
        View Calendar
      </NavLink>
    </div>
    <div className="timeline" data-testid="schedule-timeline">
      {schedule.map((event) => (
        ...
        >
          ...
          <span
            className={`timeline-dot timeline-${event.color}`}
            data-testid={`schedule-dot-${slug(event.time)}`}
          />
          <article
            className={`event-card event-${event.color}`}
            data-testid={`schedule-event-${slug(event.title)}`}
          >
            <strong data-testid={`schedule-title-${slug(event.title)}`}>
              {event.title}
            </strong>
            <span data-testid={`schedule-duration-${slug(event.title)}`}>
              {event.duration}
            </span>
            {event.focus && (
...
const FocusWidget = ({ compact = false }) => {
  ...
  return (
    <section
      className={`surface focus-widget ${compact ? "focus-compact" : ""}`}
      data-testid="focus-widget"
    >
      <div className="section-heading">
        <h2>
          <Timer size={26} weight="duotone" aria-hidden="true" /> Focus Mode
        </h2>
        <NavLink to="/focus" data-testid="focus-sessions-link">
          Sessions
        </NavLink>
      </div>
      <div
        className="timer-ring"
        style={{ "--timer-progress": `${progress}deg` }}
        data-testid="focus-timer-ring"
        aria-label="Pomodoro timer progress"
      >
        <div>
          <strong data-testid="focus-timer-value">
            {formatTimer(seconds)}
          </strong>
          <span data-testid="focus-timer-label">Pomodoro</span>
        </div>
      </div>
      <div className="focus-actions">
        <button
          ...
          data-testid="focus-start-pause-button"
        >
...
const TaskTable = ({
  ...
  return (
    <div className="task-table-wrap" data-testid="task-table-wrap">
      <table
        className="task-table enriched-task-table"
            ...
            return (
              <tr key={task.id} data-testid={`task-row-${slug(task.id)}`}>
                <td data-testid={`task-title-cell-${slug(task.id)}`}>
                  <span className={`tiny-task-icon tiny-${task.accent}`}>
                ...
                <td>
                  <button
                    ...
                    className={`today-toggle ${task.workingToday ? "active" : ""}`}
                    data-testid={`task-today-button-${slug(task.id)}`}
                  >
                    {task.workingToday ? "Working" : "Add"}
                  </button>
                </td>
                <td data-testid={`task-source-${slug(task.id)}`}>
                  {task.source}
                </td>
                <td>
                  <Pill
                    tone={task.priority.toLowerCase()}
                    testId={`task-priority-${slug(task.id)}`}
                  >
                    {task.priority}
                  </Pill>
                </td>
                <td
                  className="ai-cell"
                  data-testid={`task-ai-${slug(task.id)}`}
                >
                  <span className="ai-score">
                    {Math.round((task.priorityScore || 0) * 100)}%
                  </span>
                  <span>
                    {task.difficulty} - impact {task.impact}/10
                  </span>
                </td>
                <td data-testid={`task-time-${slug(task.id)}`}>
                  {task.time} mins
                </td>
                <td data-testid={`task-xp-${slug(task.id)}`}>{task.xp} XP</td>
                <td data-testid={`task-completed-${slug(task.id)}`}>
                  {formatDateTime(task.completedAt)}
                </td>
                <td>
                  <Pill
                    tone={slug(task.status)}
                ...
                <td className="notes-cell">
                  <textarea
                    ...
                    value={task.notes || ""}
                    onChange={(event) =>
                      onUpdateNotes(task.id, event.target.value)
                    }
                ...
                <td className="task-actions action-menu-cell">
                  ...
                  {isMenuOpen && (
                    <div
                      ...
                      role="menu"
                      data-testid={`task-action-menu-${slug(task.id)}`}
                    >
                      <button
                        role="menuitem"
                      ...
                      <button
                        role="menuitem"
                        onClick={() => runAction(() => onStatusChange(task.id))}
                        data-testid={`task-status-button-${slug(task.id)}`}
                      >
                        Advance
                      </button>
                      <button
                        role="menuitem"
                        onClick={() => runAction(() => onComplete(task.id))}
                        data-testid={`task-complete-button-${slug(task.id)}`}
                      >
                        Done
                      </button>
                    </div>
    ...
    >
      <label>
        ...
        <input
          ...
          placeholder="Investigate CI failure"
          data-testid={`${mode}-task-title-input`}
        />
      </label>
      <label>
        Description
        <textarea
          ...
          data-testid={`${mode}-task-description-input`}
        />
      </label>
      <label>
        Type
      ...
      <label>
        ...
        <input
          ...
          onChange={(event) => update("actualMinutes", event.target.value)}
        />
      </label>
      <label>
        XP
      ...
      <label className="wide-field">
        ...
        <textarea
          ...
          placeholder="Learnings, what went right, what went wrong, blockers..."
          data-testid={`${mode}-task-notes-input`}
        />
      </label>
      <label className="checkbox-field">
        <input
          ...
          onChange={(event) => update("workingToday", event.target.checked)}
        />{" "}
      ...
      <label className="checkbox-field">
        <input
          type="checkbox"
          checked={form.runAiEnrichment}
          onChange={(event) => update("runAiEnrichment", event.target.checked)}
        />{" "}
        Run AI enrichment
      </label>
      <div className="editor-actions">
        {onCancel && (
          <button
            ...
            onClick={onCancel}
            data-testid={`${mode}-task-cancel-button`}
          >
            Cancel
          </button>
        )}
        <button
          className="primary-action"
          type="submit"
          data-testid={`${mode}-task-submit-button`}
        >
...
const taskFromForm = (form, existingTask) => {
  ...
  const completedAt =
    status === "Done"
      ? existingTask?.completedAt || nowIso()
      : existingTask?.completedAt && existingTask.status === "Done"
        ? undefined
...
const completedTodayTasks = (tasks) =>
  tasks.filter((task) => task.status === "Done" && isSameDay(task.completedAt));

const generateStandupNote = (tasks) => {
  const accomplished = completedTodayTasks(tasks);
  const inProgress = tasks.filter(
    (task) =>
      task.workingToday && task.status !== "Done" && task.status !== "Blocked",
  );
  const blockers = tasks.filter((task) => task.status === "Blocked");
  const accomplishedText = accomplished.length
    ? accomplished.map((task) => task.title).join("; ")
    : "No completed tasks logged yet.";
  const inProgressText = inProgress.length
    ? inProgress.map((task) => task.title).join("; ")
    : "No active in-progress tasks selected.";
  const blockerText = blockers.length
    ? blockers
        .map((task) => `${task.title}${task.notes ? ` (${task.notes})` : ""}`)
        .join("; ")
  ...
  return {
    ...
    nextSteps: inProgress
      .slice(0, 3)
      .map((task) => task.aiInsight || `Continue ${task.title}`),
    ...
    <main className="dashboard-page" data-testid="dashboard-page">
      <section className="stats-grid" aria-label="Daily productivity metrics">
        <StatCard
          ...
          detail="Includes completed work"
          icon={Trophy}
          tone="violet"
          trend
        ...
        <StatCard
          ...
          value={`${completedCount} today`}
          detail="Completion date is captured"
          icon={CheckCircle}
          tone="blue"
          progress={Math.min(
            100,
            (completedCount / Math.max(1, todayTasks.length)) * 100,
          )}
      ...
      <div className="content-grid">
        <section
          className="surface missions-panel"
          data-testid="missions-panel"
        >
          <div className="section-heading">
            <h2>
              <Flag size={26} weight="duotone" aria-hidden="true" />{" "}
              Today&apos;s Missions
            </h2>
            <NavLink to="/quests" data-testid="view-all-missions-link">
              View quests
            </NavLink>
          </div>
        ...
        >
          <div className="section-heading task-panel-heading">
            <h2>
              <ListBullets size={26} weight="duotone" aria-hidden="true" /> My
              Tasks
            </h2>
            <NavLink
              className="add-task-link"
              to="/tasks"
              data-testid="dashboard-add-task-link"
            >
              <Plus size={19} weight="bold" aria-hidden="true" /> Add Task
            </NavLink>
          </div>
          <div className="tab-row" role="tablist" aria-label="Task filters">
            {["All", "Working Today", "Done", "Blocked"].map((tab, index) => (
              <button
                key={tab}
                className={index === 0 ? "tab active" : "tab"}
                role="tab"
                data-testid={`task-filter-${slug(tab)}`}
              >
                {tab}
              </button>
            ))}
        ...
        <aside className="right-stack" data-testid="right-stack">
          <FocusWidget compact />
          <section
          ...
          >
            ...
            <h2>
              <Sparkle size={25} weight="duotone" aria-hidden="true" /> AI
              Insight
            </h2>
          ...
          <section className="surface quote-card" data-testid="quote-card">
            <div className="quote-mark" aria-hidden="true">
              &quot;
            </div>
            <p data-testid="quote-text">
              Discipline is the bridge between goals and accomplishment.
            </p>
            <span data-testid="quote-author">- Jim Rohn</span>
          </section>
        </aside>
      </div>
      <p className="footer-note" data-testid="dashboard-footer-note">
  ...
  return (
    <main className="page-stack" data-testid="tasks-page">
      <section className="surface form-card" data-testid="add-task-card">
        <div className="section-heading">
          <h2>
            <Plus size={26} weight="duotone" aria-hidden="true" /> Add Task With
            Full Details
          </h2>
          <span data-testid="task-count-label">
            {tasks.length} tasks loaded
          </span>
        </div>
        <TaskEditor
          mode="create"
          onSubmit={(form) => onAddTask(taskFromForm(form))}
        />
      </section>
      {editingTask && (
        <section className="surface form-card" data-testid="edit-task-card">
          <div className="section-heading">
            <h2>
              <FileText size={26} weight="duotone" aria-hidden="true" /> Edit
              Task
            </h2>
            <span>{editingTask.id}</span>
          </div>
          <TaskEditor
            mode="edit"
            task={editingTask}
            onSubmit={(form) => {
              onEdit(taskFromForm(form, editingTask));
              setEditingTask(null);
            }}
      ...
      <section className="surface" data-testid="unified-task-list-card">
        <div className="section-heading">
          <h2>
            <Database size={26} weight="duotone" aria-hidden="true" /> Unified
            Task List
          </h2>
          <button className="ghost-button" data-testid="task-filter-button">
...
const CalendarPage = ({ overview }) => (
  <main className="page-stack" data-testid="calendar-page">
    <SchedulePanel />
    <section
      className="surface weekly-panel"
      data-testid="weekly-overview-card"
    >
      <div className="section-heading">
        <h2>
          <SquaresFour size={26} weight="duotone" aria-hidden="true" /> Weekly
          Overview
        </h2>
      ...
      <div className="weekly-grid">
        <StatCard
          ...
          tone="green"
          trend
          testId="weekly-completed-stat"
        />
        <StatCard
          label="XP Earned"
          value="740 XP"
          detail="Level 8 is within reach"
          icon={Trophy}
          tone="violet"
          testId="weekly-xp-stat"
        />
        <StatCard
          label="Meeting Time"
          value={formatMinutes(overview.meetingMinutes)}
          detail="Tracked from calendar"
          icon={Clock}
          tone="blue"
          trend
          testId="weekly-time-stat"
        />
      </div>
    </section>
  </main>
...
const FocusPage = () => (
  <main className="focus-page" data-testid="focus-page">
    <FocusWidget />
    <section
      className="surface focus-guidance"
      data-testid="focus-guidance-card"
    >
      <h2>
        <Lightning size={26} weight="duotone" aria-hidden="true" /> Deep Work
        Protocol
      </h2>
      <ul>
        <li data-testid="focus-guidance-item-1">
          Start with the highest-impact mission.
        </li>
        <li data-testid="focus-guidance-item-2">
          Mute distractions until the timer ends.
        </li>
        <li data-testid="focus-guidance-item-3">
          Complete one status update after each session.
        </li>
      </ul>
    </section>
  </main>
...
const QuestsPage = ({ tasks }) => {
  const todayTasks = tasks
    .filter((task) => task.workingToday)
    .sort((a, b) => (b.priorityScore || 0) - (a.priorityScore || 0));
  return (
    <main className="page-stack" data-testid="quests-page">
      <section
        className="surface missions-panel"
        data-testid="all-quests-panel"
      >
        <div className="section-heading">
          <h2>
            <Flag size={26} weight="duotone" aria-hidden="true" /> Daily Quest
            Board
          </h2>
          <button
            className="primary-action"
            data-testid="generate-quests-button"
          >
            <Sparkle size={19} weight="duotone" aria-hidden="true" /> Generate
            Quests
          </button>
        </div>
        <div className="mission-list">
          {todayTasks.length ? (
            ...
            <p className="empty-state">
              No tasks are marked as working today yet.
            </p>
          )}
        </div>
      </section>
      <section className="surface" data-testid="daily-work-source-card">
        <div className="section-heading">
          <h2>
            <Database size={26} weight="duotone" aria-hidden="true" /> Daily
            Work Source
          </h2>
          <span>{todayKey()}</span>
        </div>
        <div className="source-truth-grid">
          {todayTasks.map((task, index) => (
            <article className="source-row" key={task.id}>
              ...
              <span>
                {task.id} - {task.priority} - {task.time} mins planned -{" "}
                {task.xp} XP
              </span>
...
const InsightsPage = ({ tasks, onRefreshInsights }) => {
  const [standupNote, setStandupNote] = useState(() =>
    generateStandupNote(tasks),
  );
  const todayTasks = tasks.filter((task) => task.workingToday);
  const completed = completedTodayTasks(tasks);
  const topPriority = [...todayTasks].sort(
  ...
  return (
    <main className="page-stack" data-testid="insights-page">
      <section
        className="surface insight-detail-card"
        data-testid="capacity-analysis-card"
      >
        <div className="section-heading">
          <h2>
            <Sparkle size={26} weight="duotone" aria-hidden="true" /> AI Task
            Insights
          </h2>
          <button
            className="ghost-button"
        ...
        <div className="capacity-grid">
          <StatCard
            label="Top Priority"
            value={topPriority?.priority || "None"}
            detail={topPriority?.title || "Mark a task for today"}
            icon={Flag}
            tone="red"
            testId="capacity-top-priority-stat"
          />
          <StatCard
            ...
            detail="Working-today tasks"
            icon={Clock}
            tone="blue"
            testId="capacity-working-hours-stat"
          />
          <StatCard
            label="Today XP"
            value={`${completed.reduce((sum, task) => sum + task.xp, 0)} XP`}
            detail="Completed today"
            icon={Trophy}
            tone="green"
            testId="capacity-xp-stat"
          />
        </div>
        <div className="insight-list">
          {todayTasks.map((task) => (
            <article key={task.id}>
              <strong>{task.title}</strong>
              <span>
                {Math.round((task.priorityScore || 0) * 100)} priority -{" "}
                {task.xp} XP - {task.time} min effort
              </span>
              <p>{task.aiInsight}</p>
            </article>
          ))}
      ...
      <section
        className="surface standup-card"
        data-testid="standup-generator-card"
      >
        <div className="section-heading">
          <h2>
            <FileText size={26} weight="duotone" aria-hidden="true" /> Standup
            Note Generator
          </h2>
          <button
            className="primary-action"
            onClick={() => setStandupNote(generateStandupNote(tasks))}
            data-testid="generate-standup-button"
          >
            <Sparkle size={19} weight="duotone" aria-hidden="true" /> Generate
          </button>
        </div>
        <pre className="standup-note" data-testid="standup-summary-text">
          {standupNote.fullNote}
        </pre>
        <div className="insight-grid">
          <span>
            <strong>Accomplished</strong>
            {standupNote.accomplished}
          </span>
          <span>
            <strong>In Progress</strong>
            {standupNote.inProgress}
          </span>
          <span>
            <strong>Blockers</strong>
            {standupNote.blockers}
          </span>
        </div>
      </section>
    </main>
...
const OverviewPage = ({ tasks, overview, onOverviewChange }) => {
  ...
  const completedWeek = tasks.filter(
    (task) =>
      task.status === "Done" && isWithinWeek(task.completedAt, weekStart),
  );
  ...
  const update = (field, value) =>
    onOverviewChange({ ...overview, [field]: value });
    ...
    <main className="page-stack" data-testid="overview-page">
      <section className="surface" data-testid="daily-overview-card">
        ...
        <div className="overview-stats">
          <StatCard
            ...
            detail={`${dailyXp} XP earned`}
            icon={CheckCircle}
            tone="green"
            testId="daily-completed-stat"
          />
          <StatCard
            label="Meetings"
            value={formatMinutes(overview.meetingMinutes)}
            detail="Editable daily tracker"
            icon={UsersThree}
            tone="orange"
            testId="daily-meetings-stat"
          />
          <StatCard
            label="Focus Time"
            value={formatMinutes(overview.focusMinutes)}
            detail="Deep-work estimate"
            icon={Timer}
            tone="blue"
            testId="daily-focus-stat"
          />
        </div>
        <div className="overview-editor">
          <label>
            ...
            <input
              type="number"
              min="0"
              value={overview.meetingMinutes}
              onChange={(event) =>
                update("meetingMinutes", parseNumber(event.target.value, 0))
              }
            />
          </label>
          <label>
            ...
            <input
              type="number"
              min="0"
              value={overview.focusMinutes}
              onChange={(event) =>
                update("focusMinutes", parseNumber(event.target.value, 0))
              }
            />
          </label>
          <label>
            New learnings
            <textarea
              value={overview.newLearnings}
              onChange={(event) => update("newLearnings", event.target.value)}
            />
          </label>
          <label>
            Went well
            <textarea
              value={overview.wentWell}
          ...
          <label>
            Went wrong
            <textarea
              value={overview.wentWrong}
              onChange={(event) => update("wentWrong", event.target.value)}
            />
          </label>
        </div>
        <div className="accomplished-list">
          {completedToday.map((task) => (
            <article key={task.id}>
              ...
              <span>
                {formatDateTime(task.completedAt)} -{" "}
                {task.actualMinutes || task.time} mins - {task.xp} XP
              </span>
              <p>{task.notes}</p>
            </article>
          ))}
        </div>
        <p className="insight-copy" data-testid="daily-overview-summary">
          Summary:{" "}
          {completedToday.length
            ? `Completed ${completedToday.length} task(s), earned ${dailyXp} XP, and captured ${notes.length} task note(s) for AI review.`
            : "No completions logged for today yet."}
        </p>
      </section>
      <section className="surface" data-testid="weekly-overview-card">
        <div className="section-heading">
          ...
          <span>
            {weekStart} to {addDaysKey(weekStart, 6)}
          </span>
        </div>
        <div className="weekly-grid">
          <StatCard
            label="Completed"
            value={`${completedWeek.length} tasks`}
            detail={`${weeklyXp} XP earned`}
            icon={CheckSquare}
            tone="green"
            testId="overview-weekly-completed-stat"
          />
          <StatCard
            label="Meeting Time"
            value={formatMinutes(overview.meetingMinutes * 5)}
            detail="Projected from daily tracker"
            icon={CalendarBlank}
            tone="orange"
            testId="overview-weekly-meetings-stat"
          />
          <StatCard
            label="Focus Time"
            value={formatMinutes(overview.focusMinutes * 5)}
            detail="Projected from daily tracker"
            icon={Clock}
            tone="blue"
            testId="overview-weekly-focus-stat"
          />
        </div>
        <div className="theme-list">
          {[...new Set(completedWeek.flatMap((task) => task.labels || []))]
            .slice(0, 5)
            .map((theme) => (
              <Pill key={theme} tone="task">
                {theme}
              </Pill>
            ))}
        </div>
        <p className="insight-copy">
          ...
          {completedWeek.length
            ? `The week is trending around ${completedWeek
                .slice(0, 3)
                .map((task) => task.title)
                .join(", ")}.`
            : "Complete tasks to build the weekly summary."}
        </p>
      </section>
    </main>
  );
...
const SyncPage = () => (
  <main className="page-stack" data-testid="sync-page">
    <section className="surface sync-card" data-testid="sync-management-card">
      <div className="section-heading">
        <h2>
          <CloudArrowDown size={26} weight="duotone" aria-hidden="true" /> Sync
          Center
        </h2>
        <button className="primary-action" data-testid="run-sync-button">
          <CloudArrowDown size={19} weight="duotone" aria-hidden="true" /> Sync
          Now
        </button>
      </div>
      <div className="sync-grid">
        {["Jira", "Outlook Calendar", "Microsoft To Do"].map((source) => (
          <article
            className="sync-source"
            key={source}
            data-testid={`sync-source-${slug(source)}`}
          >
            <CheckCircle size={26} weight="duotone" aria-hidden="true" />
            <strong data-testid={`sync-source-title-${slug(source)}`}>
              {source}
            </strong>
            <span data-testid={`sync-source-status-${slug(source)}`}>
              Ready to sync
            </span>
          </article>
        ))}
...
const SettingsPage = () => (
  <main className="page-stack" data-testid="settings-page">
    <section className="surface settings-card" data-testid="settings-card">
      <div className="section-heading">
        <h2>
          <GearSix size={26} weight="duotone" aria-hidden="true" /> Productivity
          Settings
        </h2>
      </div>
      <label className="settings-row" data-testid="working-hours-setting-label">
        Working hours
        <input
          value="09:00 - 17:00"
          readOnly
          data-testid="working-hours-setting-input"
        />
      </label>
      <label className="settings-row" data-testid="xp-multiplier-setting-label">
        Focus XP multiplier
        <input
          value="1.5x"
          readOnly
          data-testid="xp-multiplier-setting-input"
        />
      </label>
    </section>
  </main>
...
const AppShell = () => {
  ...
  const handleStatusChange = (id) =>
    setTasks((items) =>
      items.map((task) => {
        ...
        const nextStatus =
          statusCycle[(currentIndex + 1) % statusCycle.length] || "To Do";
        return normalizeTask({
          ...task,
          status: nextStatus,
          completedAt:
  ...
  const handleToggleToday = (id) =>
    setTasks((items) =>
      items.map((task) =>
        task.id === id
          ? normalizeTask({ ...task, workingToday: !task.workingToday })
          : task,
      ),
    );
  const handleUpdateNotes = (id, notes) =>
    setTasks((items) =>
      items.map((task) =>
        task.id === id
          ? normalizeTask({ ...task, notes, aiInsight: "" })
          : task,
      ),
    );
  ...
  const handleRefreshInsights = () =>
    setTasks((items) =>
      items.map((task) => normalizeTask({ ...task, aiInsight: "" })),
    );
  ...
  return (
    <div className="app-shell" data-testid="app-shell">
      ...
      <button
        className={`sidebar-scrim ${sidebarOpen ? "sidebar-scrim-active" : ""}`}
        aria-label="Close navigation"
        onClick={() => setSidebarOpen(false)}
        data-testid="sidebar-scrim-button"
        ...
        <Routes>
          ...
          <Route
            ...
            element={
              <TasksPage
                tasks={tasks}
                onAddTask={(task) => setTasks((items) => [task, ...items])}
                onComplete={handleComplete}
                onStatusChange={handleStatusChange}
                onEdit={handleEditTask}
                onToggleToday={handleToggleToday}
          ...
          <Route
            path="/calendar"
            element={<CalendarPage overview={overview} />}
          />
