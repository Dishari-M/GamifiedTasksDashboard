...
- Never commit wallet zip files, `ewallet.pem`, database passwords, or local
  `.env` files.
- For the current team ADB, use the shared `DEVQUEST_APP` schema so all
  developers hit the app-owned tables and auth data. `ADMIN` is an owner/admin
  account and may contain older or partial copies of tables.
- Rotate the wallet if it is accidentally shared.
- Use a compartment, VCN, or allowed IP setup appropriate for the environment.

## Current Team Schema Note

As of the latest local check, `DEVQUEST_APP` is the runtime schema for the
integrated app. It supports the current auth, quests, focus, task, and
working-today flows. `ADMIN` is not schema-compatible with the current auth
code because its `APP_USERS` table has different identity columns.

Current seed/sync notes:

| Area | Current status |
| --- | --- |
| Daily/weekly overviews | `DAILY_OVERVIEWS` and `WEEKLY_OVERVIEWS` have been created in `DEVQUEST_APP`; rows are generated by the overview APIs. |
| Calendar demo data | `CALENDAR_EVENTS` has small May 6, 2026 demo seed rows for `user-1` and `user-13` / `admin2`. |
| User stats | `USER_STATS` has small demo rows for `user-1` and `user-13` / `admin2`. |
| AI runs | `AI_RUNS` exists in `DEVQUEST_APP`; rows are created by AI generation APIs. |

If schema sync is needed, migrate DDL into `DEVQUEST_APP` rather than changing
the app back to `ADMIN`.
